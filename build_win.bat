...
python -m nuitka --standalone --onefile --assume-yes-for-downloads --mingw64 --experimental=force-dependencies-pefile --windows-console-mode=disable --windows-icon-from-ico=icon.ico --company-name="夜雨微寒" --product-name="cursor账号管理器" --product-version=%VER% --file-version=%VER%.0 --include-package=app --include-data-dir=web=web --output-filename=%EXENAME% --output-dir=nuitka-out app/__main__.py || exit /b 1
