...
class PatchStatus:
    ...
    def installed(self) -> bool:
        return (
            ...
            + self.local_actions_markers
            + self.subagent_local_markers
            > 0
        )
    ...
    def stream_mode_installed(self) -> bool:
        ...
        return (
            ...
            and self.local_actions_markers > 0
            and self.subagent_local_markers > 0
        )
...
# 5) 本地回路准入放宽。官方 selectTurnRuntime 只让「userMessageAction + AGENT 模式 + 无子代理 run
#    options」进 managed-local，其余一律 {runtime:"connect"} 回落云端 NAL。而 connect 那条路在 sand
#    身份下被服务端 401（ERROR_NOT_LOGGED_IN）拒绝，实测表现：
#      - 每个后台命令跑完，Cursor 自动发 backgroundTaskCompletionAction → "action-not-supported"
#        → connect → 401 → 界面弹 "An unexpected error occurred. Request ID: …"；
#      - 子代理（executeSubagentTurn 带 subagentTypeName / parentAgentToolCallId）
#        → "run-options-not-supported" → connect → 401 → 子代理起不来，也用不到 Bot 额度。
#    675.js 的本地 runtime 其实已经为下面这些动作注册了 handler，并有 subagentTypeName /
#    ASK / PLAN 的处理代码，所以把准入放宽到它真正支持的范围。写法：在原 return 前插入新判定
#    并 return，原三元整段保留为死代码；卸载按 marker 精确删除，字节级可还原。
LOCAL_RUNTIME_ACTIONS: Tuple[str, ...] = (
    "userMessageAction",
    "resumeAction",
    "summarizeAction",
    "shellCommandAction",
...
LOCAL_ACTIONS_RE = re.compile(
    r"(?<!" + re.escape(SAND_LOCAL_ACTIONS_END) + r")"
    r'return"userMessageAction"!==(\w+)\.actionCase\?"action-not-supported":\1\.requestedMode!=='
)
LOCAL_ACTIONS_RESTORE_RE = re.compile(
    re.escape(SAND_LOCAL_ACTIONS_MARKER) + r"[\s\S]*?" + re.escape(SAND_LOCAL_ACTIONS_END)
)


def _local_actions_sub(match: "re.Match[str]") -> str:
    var = match.group(1)
    actions = "|".join(LOCAL_RUNTIME_ACTIONS)
    return (
        SAND_LOCAL_ACTIONS_MARKER
        + f'if(!/^(?:{actions})$/.test(String({var}.actionCase)))return"action-not-supported";'
        + f'if(void 0==={var}.modelId)return"model-not-supported";'
        + f'if({var}.hasModelCredentials)return"private-model-not-supported";'
        + f'if({var}.hasUnsupportedRunOptions)return"run-options-not-supported";return;'
        + SAND_LOCAL_ACTIONS_END
        + match.group(0)
    )


# 6) 子代理 run options 不再算「不支持」：subagentTypeName / parentAgentToolCallId /
#    directMetaParentChildSubagent 三项用 !1&&(…) 短路成假（&& 优先级高于 ||，前面的
#    customSystemPrompt / harness / excludeWorkspaceContext 判定不受影响，本地 runtime 确实不认
#    这三项，继续回落 connect）。原表达式原样保留在括号里，卸载精确还原。
SUBAGENT_RUN_OPTIONS_RE = re.compile(
    r"\|\|(void 0!==(\w+)\.runOptions\.subagentTypeName"
    r"\|\|void 0!==\2\.runOptions\.parentAgentToolCallId"
    r"\|\|!0===\2\.runOptions\.directMetaParentChildSubagent)"
)
SUBAGENT_RUN_OPTIONS_RESTORE_RE = re.compile(
    r"\|\|!1&&\((void 0!==(\w+)\.runOptions\.subagentTypeName"
    r"\|\|void 0!==\2\.runOptions\.parentAgentToolCallId"
    r"\|\|!0===\2\.runOptions\.directMetaParentChildSubagent)\)"
    + re.escape(SAND_SUBAGENT_LOCAL_MARKER)
)


def _subagent_run_options_sub(match: "re.Match[str]") -> str:
...
def apply_patch_to_content(content: str) -> Tuple[str, PatchStats]:
    ...
    next_content, local_actions_count = LOCAL_ACTIONS_RE.subn(
        _local_actions_sub, next_content
    )
    stats.local_actions += local_actions_count

    next_content, subagent_local_count = SUBAGENT_RUN_OPTIONS_RE.subn(
        _subagent_run_options_sub, next_content
    )
...
def remove_patch_from_content(content: str) -> Tuple[str, RemoveStats]:
    ...
    next_content, local_actions_count = LOCAL_ACTIONS_RESTORE_RE.subn("", next_content)
    stats.local_actions += local_actions_count

    next_content, subagent_local_count = SUBAGENT_RUN_OPTIONS_RESTORE_RE.subn(
...
def _build_install_plan(
    ...
    for target in layout.target_paths:
        ...
        total.local_actions += stats.local_actions
        total.subagent_local += stats.subagent_local
    if plan:
